...
pre_cut_num_trunc = 128 # hard code, beam search 前3层按配置的值，后3层简化计算
...
# 5.4 precut
        ...
        pre_cut_num = pre_cut_num_list[index] if index < 3 else pre_cut_num_trunc
...
# 5.6 update kv cache
        if args.self_att_kv_cache:
            ...
            for i in range(model_config.num_block):
                cache_K, cache_V = self_att_kv_cache["self_att_kv_cache_" + str(i)]
                cache_K = tf.gather(cache_K, kv_indices)
                cache_V = tf.gather(cache_V, kv_indices)
                self_att_kv_cache["self_att_kv_cache_" + str(i)] = [cache_K, cache_V]
