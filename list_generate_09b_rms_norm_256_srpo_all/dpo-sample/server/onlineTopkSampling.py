...
service_name = "grpc_list09b_dpo_sample"
...
class GenerativeRetrFlow(TowerPredictFlowBase,PDNApiMixin, CofeaApiMixin,UniPredictApiMixin):
    ...
    def retrieve(self, **kwargs):
        ...
        return (
            ...
            .if_("request_type == 'end2end' or request_type == 'eval_request' or request_type == 'dpo_request'")
                ...
                .dispatch_common_attr(
                    dispatch_config = [
                        ...
                        "from_common_attr": "sid_list_list",
                        "to_item_attr" : "session_sid_list",
                        "by_list_size": max_target_num*3,
                        }
                    ]
                )
                .enrich_attr_by_lua(
...
dpo_pre_flow = (GenerativeRetrFlow(name="dpo_pre_flow")
    .limit(0)
    .copy_user_meta_info(
        save_user_id_to_attr="user_id",
        save_request_time_to_attr="req_time"
    )
    .log_debug_info(common_attrs=["user_id", "req_time"], for_debug_request_only=False)
    .gen_common_attr_by_lua(attr_map={"is_execute_dpo": "math.random() < dpo_ratio"}) # 3% dpo 采样
    .perflog_attr_value(check_point="dpo_pre_flow", common_attrs=["is_rlhf_train", "is_execute_dpo"])
    .if_("is_execute_dpo ~= 1")
        .enrich_attr_by_lua(
            export_common_attr=["dpo_pos", "dpo_neg", "is_dpo_ok", "ref_pos_logp", "ref_neg_logp"],
            function_for_common="calculate",
            lua_script=f"""
            function calculate()
                local dpo_pos = {{}}
                ...
                local ref_pos_logp = {{}}
                local ref_neg_logp = {{}}
                {EOL.join(f"table.insert(dpo_pos, 0)" for x in range(max_target_num*3))}
                {EOL.join(f"table.insert(dpo_neg, 0)" for x in range(max_target_num*3))}
                local is_dpo_ok = {{}}
                table.insert(is_dpo_ok, 0)

                table.insert(ref_pos_logp, 0.0)
                table.insert(ref_neg_logp, 0.0)

                return dpo_pos, dpo_neg, is_dpo_ok, ref_pos_logp, ref_neg_logp
            end
            """
        )
        .return_()
...
# 奖励模型评估
return_pxtrs = ["seq_wtd", "seq_ltr", "seq_cmtr", "seq_wtr", "seq_ftr", "seq_evtr", "seq_vtr", "seq_swt", "seq_lsst"]
reward_model_eval_flow = (GenerativeRetrFlow(name="reward_model_eval_flow")
    .if_("request_type == 'eval_request' or request_type == 'dpo_request'")
        .if_("is_reward_model_on == 1")
            ...
            .set_attr_value(
                no_overwrite=True,
                common_attrs=[{ "name": "retr_type", "type": "int", "value": 1 }]
            )
            .delegate_enrich(
            ...
            .enrich_attr_by_lua(
                import_item_attr=["f1_score_for_train"],
                export_item_attr=["f1_score"],
                function_for_item="calc",
                lua_script="""
                    function calc()
                        return f1_score_for_train
                    end
                """
            )
            .enrich_attr_by_lua(
            ...
            .gen_common_attr_by_lua(attr_map={"rotate_ratio": "math.random()"})
            .gen_common_attr_by_lua(attr_map={"multi_target_seq_ltr_ratio": "0.0"})
            .gen_common_attr_by_lua(attr_map={"multi_target_seq_cmtr_ratio": "0.0"})
            .gen_common_attr_by_lua(attr_map={"multi_target_seq_wtr_ratio": "0.0"})
            .gen_common_attr_by_lua(attr_map={"multi_target_seq_swt_ratio": "0.0"})
            .gen_common_attr_by_lua(attr_map={"multi_target_seq_lsst_ratio": "0.0"})
            .if_("rotate_ratio >= swt_low_bound and rotate_ratio < swt_up_bound")
                .sort_by("seq_swt")
                .set_attr_value(common_attrs=[{"name": "multi_target_seq_swt_ratio", "type": "double", "value": 1.0}])
            .end_()
            .if_("rotate_ratio >= ltr_low_bound and rotate_ratio < ltr_up_bound")
                .sort_by("seq_ltr")
                .set_attr_value(common_attrs=[{"name": "multi_target_seq_ltr_ratio", "type": "double", "value": 1.0}])
            .end_()
            .if_("rotate_ratio >= wtr_low_bound and rotate_ratio < wtr_up_bound")
                .sort_by("seq_wtr")
                .set_attr_value(common_attrs=[{"name": "multi_target_seq_wtr_ratio", "type": "double", "value": 1.0}])
            .end_()
            .if_("rotate_ratio >= cmtr_low_bound and rotate_ratio < cmtr_up_bound")
                .sort_by("seq_cmtr")
                .set_attr_value(common_attrs=[{"name": "multi_target_seq_cmtr_ratio", "type": "double", "value": 1.0}])
            .end_()
            .if_("rotate_ratio >= lsst_low_bound and rotate_ratio < lsst_up_bound")
                .sort_by("seq_lsst")
                .set_attr_value(common_attrs=[{"name": "multi_target_seq_lsst_ratio", "type": "double", "value": 1.0}])
            .end_()
            .pack_item_attr(
                item_source={ "reco_results": True },
                mappings=[
                    {"from_item_attr": "session_sid_list", "to_common_attr": "session_sid_list_pack" },
                    {"from_item_attr": "session_score", "to_common_attr": "session_score_list_pack"},
                    {"from_item_attr": "f1_score", "to_common_attr": "f1_score_pack"},
                ]
            )
            .gen_common_attr_by_lua(attr_map={"is_dpo_ok": "{0}"})
            .enrich_attr_by_lua( # 取 top1 为正样本，tail1 为 负样本
                import_common_attr=["session_sid_list_pack", "session_score_list_pack"],
                export_common_attr=["dpo_pos", "dpo_neg", "is_dpo_ok", "ref_pos_logp", "ref_neg_logp"],
                function_for_common="calculate",
                lua_script=f"""
                function calculate()
                    ...
                    local ref_pos_logp = {{}}
                    local ref_neg_logp = {{}}
                    local is_dpo_ok = is_dpo_ok or {{}}
                    local item_num = #session_sid_list_pack / ({max_target_num} * 3);  --list 个数
                    local tail = (item_num - 1) * ({max_target_num} * 3)+1
                    local cnt = 0
                    for i=1, #session_sid_list_pack, {max_target_num} * 3 do
                        cnt = cnt + 1
                        if i == 1 then
                            {EOL.join(f"table.insert(dpo_pos, session_sid_list_pack[i+{x}])" for x in range(max_target_num*3))}
                            table.insert(ref_pos_logp, session_score_list_pack[cnt])
                        end

                        if i == tail then
                            {EOL.join(f"table.insert(dpo_neg, session_sid_list_pack[i+{x}])" for x in range(max_target_num*3))}
                            table.insert(ref_neg_logp, session_score_list_pack[cnt])
                        end
                    end
            ...
            .enrich_attr_by_lua(
                import_common_attr=["ref_pos_logp", "ref_neg_logp"],
                export_common_attr=["is_valid_dpo_pair"],
                function_for_common="calc",
                lua_script="""
                    function calc()
                        local ref_pos_session_score = ref_pos_logp[1]
                        local ref_neg_session_score = ref_neg_logp[1]
                        -- 如果 reward 分数高的 list, 生成分数仍然高, 则说明无效样本
                        if ref_pos_session_score >= ref_neg_session_score then
                            return 0.0;
                        else
                            return 1.0;
                        end
            ...
            .perf_reward_value(namespace="common.leaf", subtag="dpo.reward_value", service_name=service_name)
            .perflog_attr_value(
                check_point="reward.seqPxtr",
                common_attrs=["list_after_zero_filter_num", "is_valid_dpo_pair"] + ["multi_target_seq_swt_ratio", "multi_target_seq_ltr_ratio", "multi_target_seq_wtr_ratio", "multi_target_seq_cmtr_ratio", "multi_target_seq_lsst_ratio"],
                item_attrs=[f"{x}" for x in return_pxtrs] + ["f1_score"]
            )
        .end_()
    .end_()
)
...
service = LeafService(
    ...
    common_attrs_from_request=["user", "disable_nearline_cache", "eval_pos_photo_id_list", "tab_id", "extra_label_prompt_input",
        "sampling_temperature_from_req", "top_k_sampling_k_from_req", "return_m_from_req", "is_rlhf_train"],
...
service.IGNORE_UNUSED_ATTR = [
    ...
    "return_m_input_out", "top_k_sampling_k_input_out", "session_photo_id_list", "session_sid_list"
]
service.IGNORE_NO_SOURCE_ATTR=["browsed_pids", "session_photo_id_list", "session_sid_list", "session_score"]
...
service.return_common_attrs(attrs=["dpo_pos", "dpo_neg", "is_dpo_ok", "ref_pos_logp", "ref_neg_logp"])
...
service.add_leaf_flows(request_type="dpo_request", leaf_flows=[get_kconf_ab_flow, dpo_pre_flow, colossus_flow, retr_flow, reward_model_eval_flow])
