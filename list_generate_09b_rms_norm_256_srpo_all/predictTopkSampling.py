...
def rms_norm_fp32(x, eps=1e-8, p=-1., bias=False, scope=None):
    with tf.variable_scope(scope or "rms_norm"):
        x = tf.cast(x, dtype=tf.float32)
        layer_size = x.get_shape().as_list()[-1]
...
lookup_table_list = tf.cast(lookup_table_list, tf.float32)
...
path_probs = tf.ones([return_m, 1], dtype=tf.float32)
path_trunc_probs = tf.ones([return_m, 1], tf.float32)
path_logits = tf.ones([return_m, 1], tf.float32)
            ...
            for i in range(model_config.num_block):
                with tf.variable_scope("num_blocks_%d" % i):
                    ...
                    if i == model_config.num_block - 1:
                        dec = tf.cast(dec, tf.float32) + tf.clip_by_value(tf.cast(MoeLayer(model_args, name="moe_ffn_%d" % i) (rms_norm(dec)), tf.float32), -65504.0, 65504.0)
                    else:
...
# 5.3 topk 采样
            ...
            decoder_outputs_last = rms_norm_fp32(decoder_outputs_last)
        ...
        logits /= tf.cast(sampling_temperature[index], tf.float32)
...
# 5.4 构造 next step input
        selected_embeddings = tf.cast(tf.nn.embedding_lookup(token_table, selected_codebook_indices),tf.float16)
