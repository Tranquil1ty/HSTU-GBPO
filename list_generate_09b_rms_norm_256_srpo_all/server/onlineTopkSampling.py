...
# 新的命名方式
colossusdb_embd_service_name = "user_doc_0_3b_base_emb"
colossusdb_embd_table_name = "grpc_clsdb_ps-user-doc-dpo-emb7"
service_name = "grpc_list09b_predict_srpo_all"
kess_name = service_name
emb_shards = 16
queue_prefix = "list_generate_09b_srpo_all"
history_view_num = 256
max_target_num = 5
model_key = "list_generate_09b_srpo_all"
code_len = 3
EOL="\n"
abtest_config = {}
...
class GenerativeRetrFlow(TowerPredictFlowBase,PDNApiMixin, CofeaApiMixin,UniPredictApiMixin):
    ...
    def perf_deversity(self, namespace, subtag, service_name):
        self.enrich_attr_by_lua(
            import_item_attr=["session_tag_list"],
            export_item_attr=["hetu_cnt", "hetu_cnt_float"],
            function_for_item="calc_deversity",
            lua_script="""
                function calc_deversity()
                    local unique_values = {}
                    for i= 1, #session_tag_list do
                        unique_values[session_tag_list[i]] = true
                    end
                    local count = 0
        ...
        .enrich_attr_by_lua(
            import_item_attr=["session_tag_list"],
            export_item_attr=["ils_diversity", "ils_diversity_float"],
            function_for_item="calc_deversity",
            lua_script="""
                function calc_deversity()
                    local k = #session_tag_list;
                    local sim = 0;
                    for i=1, #session_tag_list-1 do
                        for j=i+1, #session_tag_list do
                            if session_tag_list[i] == session_tag_list[j] then
                                sim = sim + 1;
                            end
                        end
                    end
                    local ils = 2/(k*(k-1)) * sim
                    local ils_diversity = 1 - ils;
                    return math.floor(ils_diversity * 10000), ils_diversity * 10000.0;
                end
            """
        ) \
        .pack_item_attr(
            item_source={ "reco_results": True },
            mappings=[
                {"from_item_attr": "hetu_cnt", "to_common_attr": "hetu_cnt_MAX", "aggregator": "max"},
                {"from_item_attr": "hetu_cnt_float", "to_common_attr": "hetu_cnt_AVG", "aggregator": "avg"},
                {"from_item_attr": "ils_diversity", "to_common_attr": "ils_diversity_MAX", "aggregator": "max"},
                {"from_item_attr": "ils_diversity_float", "to_common_attr": "ils_diversity_AVG", "aggregator": "avg"},
            ]
        ) \
        .perflog(
            mode="interval",
        ...
        .perflog(
            mode="interval",
            value="{{ils_diversity_MAX}}",
            namespace=namespace,
            subtag=subtag,
            extra1=service_name,
            extra2="ils_diversity_MAX",
            extra3="{{tab_id_str}}"
        ) \
        .perflog(
            mode="interval",
            value="{{ils_diversity_AVG}}",
            namespace=namespace,
            subtag=subtag,
            extra1=service_name,
            extra2="ils_diversity_AVG",
            extra3="{{tab_id_str}}"
        )
        return self
    ...
    def retrieve(self, **kwargs):
        ...
        return (
            ...
            .get_item_attr_by_local_attr_index(
                attrs=["hetu_tag_level_info__hetu_level_one"],
                additional_item_source={
                    "reco_results": False,
                    "common_attr": ["retrieval_photo_id_list"],
                },
            )
            .enrich_attr_by_lua(
                item_list_from_attr="retrieval_photo_id_list",
                import_item_attr=["hetu_tag_level_info__hetu_level_one"],
                export_item_attr=["hetu_tag_level_info__hetu_level_one"],
                function_for_item="calc",
                lua_script="""
                    function calc()
                        if hetu_tag_level_info__hetu_level_one == nil or #hetu_tag_level_info__hetu_level_one == 0 then
                            return {-1}
                        else
                            return {hetu_tag_level_info__hetu_level_one[1]}
                        end
                    end
                """
            )
            .pack_item_attr(
                item_source={ "reco_results": False , "common_attr": ["retrieval_photo_id_list"],},
                mappings=[
                    {"from_item_attr": "hetu_tag_level_info__hetu_level_one", "to_common_attr": "tag_list" },
                ]
            )
            .limit(0)
            ...
            .if_("request_type == 'end2end' or request_type == 'eval_request'")
                ...
                .dispatch_common_attr(
                    dispatch_config = [
                        ...
                        "from_common_attr" : "tag_list",
                        "to_item_attr" : "session_tag_list",
                        "by_list_size": max_target_num,
                        },
                    ]
                )
...
reward_model_eval_flow = (GenerativeRetrFlow(name="reward_model_eval_flow")
    .if_("request_type == 'eval_request'")
        .if_("is_reward_model_on == 1")
            ...
            .perf_deversity(namespace="common.leaf", subtag="deversity", service_name=service_name)
            .set_attr_value(
                no_overwrite=True,
                common_attrs=[
