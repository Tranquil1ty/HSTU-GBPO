        ...
        def get_dense_fea(self, name, dim=1, dtype=tf.float32):
            ...
            with tf.variable_scope(f"feature_{name}"):
                if dtype == tf.int64 or dtype == tf.int32:
                    random_feature = tf.random.uniform(shape, minval=0, maxval=15, dtype=dtype, name=name)
                else:
...
if args.mode == "train":
    ...
    masked_tokens = config.get_dense_fea("semantic_id_v2", dim=senmantic_id_token_num, dtype=tf.int64)
    labels_map = {
...
def build_cross_former_self_layers(q, kv, cross_ffn_dim, self_ffn_dim, hidden_dim, 
    ...
    pos_embedding = tf.get_variable(f"{name}_pos_embedding", (senmantic_id_token_num + 1, emb_size))[:seq_len, :]
    pos_embedding = tf.tile(tf.expand_dims(pos_embedding, 0), [batch_size, 1, 1])
...
def fr_model():
    ...
    with tf_name_scope("qformer"), new_xla_jit_context():
        ...
        qformer_res = build_cross_former_self_layers(item_token_embedding, 
                                                     ...
                                                     kv_mask=None, dropout_rate=0.0)
        # shift hidden state
        qformer_res = tf.split(qformer_res, [item_seq_len, 1], axis=1)[0]
...
def predict():
    ...
    for i in range(item_seq_len):
        ...
        with tf_name_scope("pre_process"), new_xla_jit_context():
            bos_token_emb = tf.get_variable(f"qformer_bos_token_emb", (1, 1, hidden_dim))
            if i == 0:
                # bos_token_emb = tf.tile(bos_token_emb, [1, 1, 1])
                ...
                item_token_embedding = tf.concat([bos_token_emb, item_token_embedding], axis=1)
        ...
        # beam search
        # item_pred_logits = item_pred_logits[:, -1, :]  #[bs, vocab_size]
        token_probs = tf.nn.log_softmax(item_pred_logits)
        token_probs, token_indices = tf.nn.top_k(token_probs, k=pre_cut_num) # save computation  [1, n] or [topk, n]
        if i == 0:
            topk_probs, topk_indices = tf.nn.top_k(token_probs, k=beam_width) # [1, topk] or [topk, topk]
            topk_indices = tf.gather(token_indices, topk_indices, axis=1, batch_dims=1)
            ...
            path_tokens = tf.tile(path_tokens, [1, pre_cut_num, 1])
            path_tokens = tf.concat([path_tokens, tf.expand_dims(token_indices, -1)], axis=2) # [topk, pre_cut_num, token_num] + [topk, pre_cut_num, 1]
        ...
        print(semantic_id_v2.get_shape())
...
if args.mode == "train":
    ...
    if args.with_kai_v2 and not args.local_debug:
        ...
        # 打印所有节点名称，过滤出包含"Reshape"的节点
        for node in graph.get_operations():
            if "Reshape" in node.name:
                print(f"节点名: {node.name}")
                print(f"输入张量: {node.inputs}")
                print(f"输出张量: {node.outputs}")
                print("------")
    ...
    if args.dryrun and not args.local_debug:
        ...
        config.dump_predict_config(
            "./predict/3token_beam64", targets, input_type=3, extra_preds=q_names, text=args.text)
    else:
        init_op = tf.global_variables_initializer()
        sess_config = tf.ConfigProto()
    ...
    if return_type == "logits":
        ...
        targets = [
            ("topk_beam_prob", topk_beam_prob),
            ("topk_prob", topk_prob), 
            ("topk_indices", topk_indices),
            ("item_pred_logits", tf.reshape(tf.nn.softmax(item_pred_logits / p_temp, axis=-1)[:, :, 0], (batch_size, -1))),
            ("token_prob", token_prob),
            ("item_logit_idx", cur_step),
        ]
    ...
    if args.dryrun and not args.local_debug:
        ...
        config.dump_predict_config(
            "./predict/causal_large_config_3token", targets, input_type=3, extra_preds=q_names, text=args.text)
    else:
        init_op = tf.global_variables_initializer()
        sess_config = tf.ConfigProto()
        sess_config.gpu_options.allow_growth = True
