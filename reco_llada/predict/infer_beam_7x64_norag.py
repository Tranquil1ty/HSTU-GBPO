...
from sample_strategy_7x64 import ChooseTokenStrategy, RemaskTokenStrategy
...
class PredictServerFlow(LeafFlow, CommonApiMixin, KuibaApiMixin, MioApiMixin, UniPredictApiMixin, EmbeddingApiMixin, KgnnApiMixin):
    ...
    def prepare_infer_params(self):
        ...
        self.get_kconf_params(
            kconf_configs=[
                {
                    "kconf_key": "reco.model2.reco_llada_params_7x64",
    ...
    def infer(self):
        ...
        self.log_debug_info(
            log_tag="rag_item_seq",
            item_attrs=["item_rag_slots", "item_rag_parameters"],
            for_debug_request_only=True,
            respect_sample_logging=False,
        )
        ...
        executor_config = dict(
            context_per_device=context_per_device,
            memory_size_per_context=1024 * 4,
        )
    ...
    def retrieve_from_tokens(self):
        ...
        self.enrich_attr_by_lua(
            ...
            export_item_attr=["retrieve_val"],
            function_for_item="func",
            lua_script="""
            function string_hash(tokens)
                local sn = 0
                for i=1, #tokens do
                    sn = sn * 67 + tokens[i] + 1
                end
                local high_bits = 1234
                local low_bits = sn & 0xFFFFFFFFFFFF
                local result = (high_bits << 48 & 0xFFFF000000000000) | low_bits
                return result
            end

            function func()
                return string_hash(token_indices)
            end
            """
        )
        ...
        self.enrich_attr_by_lua(
            ...
            export_item_attr=["pid"],
            function_for_item="func",
            lua_script="""
            function func()
                if pids ~= nil and #pids > 0 then
                    return pids[1]
                else
                    return nil
                end
        ...
        self.dispatch_common_attr(
            dispatch_config = [
                {"from_common_attr" : "tokens", "to_item_attr" : "tokens", "by_list_size": 7}
            ]
        )
        self.end_if_()
...
service.return_item_attrs(["pid", "ann_score", "src_item", "tokens"])
