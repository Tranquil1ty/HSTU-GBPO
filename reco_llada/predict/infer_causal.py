...
class PredictServerFlow(LeafFlow, CommonApiMixin, KuibaApiMixin, MioApiMixin, UniPredictApiMixin, EmbeddingApiMixin):
    ...
    def infer_step(self):
        ...
        self.enrich_attr_by_lua(
            function_for_item="calculate",
            import_item_attr=["semantic_id_v2"],
            export_item_attr=["semantic_id_v2_prob"],
            lua_script="""
                function calculate()
        ...
        self.enrich_attr_by_lua(
            function_for_item="calculate",
            import_item_attr=["token_indices", "token_probs", "semantic_id_v2", "semantic_id_v2_prob"],
            import_common_attr=["current_step"],
            export_item_attr=["semantic_id_v2", "semantic_id_v2_prob"],
            lua_script="""
                function calculate()
                    local cur_step_id = current_step + 1
                    local semantic_id_v2_out = {}
                    local semantic_id_v2_prob_out = {}
                    for i = 1, #token_indices do
                        if i == cur_step_id then
                            table.insert(semantic_id_v2_out, token_indices[i] * 1.0)
                            table.insert(semantic_id_v2_prob_out, token_probs[i])
                        else
                            table.insert(semantic_id_v2_out, semantic_id_v2[i])
                            table.insert(semantic_id_v2_prob_out, semantic_id_v2_prob[i])
                        end
                    end
                    return semantic_id_v2_out, semantic_id_v2_prob_out
                end
            """
        )
