...
def cast_16_wrapper(func):
    def new_func(*args, **kwargs):
        x=func(*args, **kwargs, dtype=tf.float16, trainable=False)
        return x #cast_16(x)
    return new_func
...
if args.mode == "train":
    ...
    semantic_id_v2 = config.get_dense_fea("semantic_id_v2", dim=senmantic_id_token_num, dtype=tf.float16 if args.fp16 else tf.float32)
    labels = [
        config.get_dense_fea('adp_effective_view_fix', 1, dtype=tf.float16 if args.fp16 else tf.float32),
        config.get_dense_fea("click", 1, dtype=tf.float16 if args.fp16 else tf.float32),
        config.get_dense_fea("like", 1, dtype=tf.float16 if args.fp16 else tf.float32),
        config.get_dense_fea("follow", 1, dtype=tf.float16 if args.fp16 else tf.float32),
        config.get_dense_fea("forward", 1, dtype=tf.float16 if args.fp16 else tf.float32),
        config.get_dense_fea("comment", 1, dtype=tf.float16 if args.fp16 else tf.float32),
        config.get_dense_fea("long_view", 1, dtype=tf.float16 if args.fp16 else tf.float32),
        config.get_dense_fea("short_view", 1, dtype=tf.float16 if args.fp16 else tf.float32),
        config.get_dense_fea("play_complete", 1, dtype=tf.float16 if args.fp16 else tf.float32)
    ]
    semantic_id_v2 = tf.reshape(tf.cast(semantic_id_v2, tf.int32), (-1, senmantic_id_token_num))    
...
def rms_norm(x, scope_name, eps=1e-6):
    if args.mode == "predict" and args.fp16:
        ...
        return tf.cast(scale * x * tf.rsqrt(mean_square + eps), tf.float16)
    else:
        scale = tf.get_variable(f'scale_{scope_name}', x.shape.as_list()[-1], initializer=tf.ones_initializer())
        mean_square = tf.reduce_mean(x ** 2, -1, keep_dims=True)
...
def attention_layer(query, key, value, num_heads, att_emb_size, name, kv_mask=None):
    ...
    if args.mode == "predict" and args.fp16 is True:
...
if args.mode == "train":
    ...
    if return_type == "logits":
        ...
        if args.mode == "predict" and args.fp16:
            cur_step = config.get_dense_fea("cur_step", 1, dtype=tf.float16)
            batch_mask = config.get_dense_fea("batch_mask", 1, dtype=tf.float16)
            p_temp = config.get_dense_fea("p_temp", 1, dtype=tf.float16)
            beam_prob = config.get_dense_fea("beam_prob", 1, dtype=tf.float16) # [b, 1]

            cur_step = tf.cast(cur_step, tf.int32)
            batch_mask = tf.cast(batch_mask, tf.float32)
            p_temp = tf.cast(p_temp, tf.float32)
            beam_prob = tf.cast(beam_prob, tf.float32)
