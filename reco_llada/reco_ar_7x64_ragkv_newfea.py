...
rag_node_embedding_2 = config.new_embedding("rag_node_embedding_2", dim=128, expand=item_rag_size, slots=[item_rag_magic_num + 6678])
rag_node_embedding_2 = tf.reshape(rag_node_embedding_2, (-1, item_rag_size, 128))
rag_node_embedding = tf.concat([rag_node_embedding, rag_node_embedding_2], axis=-1)
...
def prepare_item_gsu_ctx_2(d_model):
    bs = tf.shape(rag_node_embedding)[0]
    rne_dim = rag_node_embedding.get_shape()[-1]
    rag_list = [rag_item_local_life_emb, rag_item_embedding_64, 
                rag_item_embedding_8, rag_item_nebula_stats]
    rag_node_pid_emb = tf.concat(rag_list, axis=-1) # [b, item_rag_size, dim]
    rag_node_pid_emb = tf.reshape(rag_node_pid_emb, (bs, rag_seq_len, token_rag_size, rag_node_pid_emb.get_shape()[-1])) # [b, item_rag_size, dim]

    rag_node_pid_emb_list = tf.split(rag_node_pid_emb, rag_seq_len, axis=1) # [b, token_rag_size, -1]

    rag_node_embedding_proj = mio_dense_layer(tf.reshape(rag_node_embedding, (bs, rag_seq_len, token_rag_size, rne_dim)), 
                                              d_model, None, "rag_node_embedding_proj", "rag_node_embedding_proj_param", bias=False)
    rag_node_embedding_list = tf.split(rag_node_embedding_proj, rag_seq_len, axis=1) # [b, token_rag_size, d_model]

    rag_item_list = []  
    with tf.variable_scope("rag_item_list", reuse=tf.AUTO_REUSE):
        for i in range(rag_seq_len):
            if i < pid_fea_seq_size:
                rag_item_list.append(rag_node_embedding_list[i])
            else:
                pid_info_emb = mio_dense_layer(rag_node_pid_emb_list[i], d_model, None, f"rag_item_merge", f"rag_item_merge_param", bias=False)
                rag_item = pid_info_emb + rag_node_embedding_list[i]
                rag_item_list.append(rag_item)

    rag_items = tf.concat(rag_item_list, axis=1) # [b, rag_seq_len * token_rag_size, d_model]
    rag_items = tf.reshape(rag_items, (bs, rag_seq_len * token_rag_size, d_model))
    return rag_items
...
def build_cross_former_self_layers(q, kv, rag_kv, cross_ffn_dim, self_ffn_dim, hidden_dim, 
    ...
    rag_mask = tf.ones((rag_seq_len), dtype=tf.float32)
    rag_mask = tf.linalg.diag(rag_mask)
    ...
    # rag_mask = P(rag_mask, "rag_mask", summarize=1024, enable_print_in_training=True)
...
if args.mode == "train":
    ...
    if args.with_kai_v2 and not args.local_debug:
        ...
        sparse_optimizer = config.optimizer.Adam(0.00005) # freeze embedding
        dense_optimizer_bias = config.optimizer.Adam(0.0001)
        dense_optimizer_mlp = config.optimizer.AdamW(learning_rate=0.0001, weight_decay=0.001)
        dense_var_bias_list = []
        dense_var_mlp_list = []
