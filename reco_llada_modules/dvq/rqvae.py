...
from data.direct_read_data import EmbeddingData, UserSeqData
...
from model.dense_enc_dec import FeedForward, EncoderDecoder
from model.quantize import VQVAEQuantize, GumbelQuantize, RQVAEQuantize
...
class RQVAE(pl.LightningModule):
    ...
    def __init__(self, args):
        ...
        self.encoder = EncoderDecoder(args.input_dim, [256, 128, 64])
        self.decoder = EncoderDecoder(args.embedding_dim, [128, 256, 512])

        # the quantizer module sandwiched between them, +contributes a KL(posterior || prior) loss to ELBO
        self.quantizer = RQVAEQuantize(args.embedding_dim)
    ...
    def forward(self, x):
        ...
        z = z.reshape([-1, 1, 1, self.args.embedding_dim]) # [B, C, H, W]
    ...
    def decode(self, token):
        z_q = self.quantizer.embed_code(token.unsqueeze(1)) # [bs, 1, n_token, c]
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.add_argument("--n_token", type=int, default=32, help="number of qunatized tokens")
        return parser
...
def cli_main():
    ...
    # model related
    parser = RQVAE.add_model_specific_args(parser)
    # dataloader related
