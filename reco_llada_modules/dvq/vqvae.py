...
from model.dense_enc_dec import FeedForward
from model.quantize import VQVAEQuantize, GumbelQuantize
from model.loss import IANormal as Normal, LogitLaplace
...
class VQVAE(pl.LightningModule):
    ...
    def __init__(self, args):
        ...
        self.encoder = FeedForward(args.input_dim, args.n_hid, args.embedding_dim * args.n_token)
        self.decoder = FeedForward(args.embedding_dim * args.n_token, args.n_hid, args.input_dim)
        ...
        self.quantizer = QuantizerModule(args.embedding_dim, args.num_embeddings, args.embedding_dim)
    ...
    def forward(self, x):
        z = self.encoder(x) # [bs, dim]
        z = z.reshape([-1, self.args.n_token, self.args.embedding_dim]) #[bs, n_token, dim]
        z_q, latent_loss, ind = self.quantizer(z)
        z_q = z_q.flatten(1) # [bs, dim]
        x_hat = self.decoder(z_q)
        return x_hat, latent_loss, ind
    
    def decode(self, token):
        z_q = self.quantizer.embed_code(token) # [bs, n_token, dim]
        z_q = z_q.flatten(1)
        x_hat = self.decoder(z_q)
        return x_hat

    def mask_decode(self, token, idx):
        z_q = self.quantizer.embed_code(token) # [bs, n_token, dim]
        B, N, D = z_q.shape
        mask = torch.ones([N], dtype=torch.int32, device=z_q.device)
        mask[idx] = 0
        mask = mask.unsqueeze(0).unsqueeze(-1).expand_as(z_q)
        z_q = z_q * mask
        z_q = z_q.flatten(1)
        x_hat = self.decoder(z_q)
        return x_hat

    def infer(self, token):
        token = token.reshape(-1, 16).to(torch.int32)
        # token = token.to(torch.int32)
        z_q = self.quantizer.embed_code(token) # [bs, n_token, dim]
        z_q_flatten = z_q.flatten(1)
        mask = (token != -1).to(torch.float32).unsqueeze(-1) #[bs, n_token, 1]
        z_q_masked = z_q * mask
        z_q_masked = z_q_masked.flatten(1)
        x_hat = self.decoder(z_q_masked)
        # x_hat = x_hat.reshape(-1, 128)
        return x_hat, z_q_flatten
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log('train_recon_loss', recon_loss, prog_bar=True, sync_dist=True)
        self.log('train_latent_loss', latent_loss, prog_bar=True, sync_dist=True)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        cluster_use = torch.sum(avg_probs > 0).to(torch.float32)
        self.log('val_perplexity', perplexity, prog_bar=True, sync_dist=True)
        self.log('val_cluster_use', cluster_use, prog_bar=True, sync_dist=True)
...
def cli_main():
    ...
    # parser.add_argument("--data_dir", type=str, default='/llm_reco_ssd/luoxinchen/dinov2/features/ia_emb_short1k_20241206')
    ...
    data = ParquetData(args.data_dir, args.batch_size, args.num_workers)
    # data = UserSeqData(
