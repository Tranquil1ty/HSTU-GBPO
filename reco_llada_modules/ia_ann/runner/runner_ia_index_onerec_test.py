...
btq_config = {
    "slot": 17,
    "queue_prefix":'ia_emb_128_for_ann',
    "shard_num": 4
}

class I2IRunnerFlow(
    LeafFlow, OfflineApiMixin, KgnnApiMixin, SwingApiMixin, KuibaApiMixin, MioApiMixin, GsuApiMixin, EmbedCalcApiMixin, PDNApiMixin, CofeaApiMixin, EmbeddingApiMixin
):
    # NEW_ROUND
    ...
    def _send_btq_embedding(self, btq_config):
        ...
        for i in range(btq_config["shard_num"]):
            self.send_with_btq(
                common_attr=f"update_message{i}",
                queue_name=f"{btq_config['queue_prefix']}{i}",
            ) 
        self.log_debug_info(
            for_debug_request_only=True,
    ...
    def _get_remote_embedding(self, emb_server_config=None):
        ...
        self.fetch_remote_embedding(
            protocol=1,
            colossusdb_embd_model_name=emb_server_config["colossusdb_embd_model_name"],
            colossusdb_embd_table_name=emb_server_config["colossusdb_embd_table_name"],
        ...
        self.pack_attr_to_bytes(
            is_common=False,
            schema=[
                dict(attr_name="photo_emb", dtype="int16"),
            ],
            output_attr_name="bytes"
        )
        self.unpack_bytes_to_attr(
            is_common=False,
            is_array=True,
            source_attr_name="bytes",
            schema=[
                dict(attr_name="photo_emb", dtype="fp16"),
            ]
        )
        self.filter_by_attr(
            attr_name = "photo_emb",
