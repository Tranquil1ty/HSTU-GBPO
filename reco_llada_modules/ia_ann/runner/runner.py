...
ann_kess = "grpc_AnnBigCodeEmd"
kgnn_kess = "grpc_kgnn_i2i_bigcode-I2I"
...
class I2IRunnerFlow(
    ...
    def _retrieve_by_colossus(self, **kwargs):
        colossus_seq_config = {
            "colossus_seq_inev_threshold": 3,
            "colossus_seq_ev_threshold": 7,
            "colossus_seq_lv_threshold": 18,
            "colossus_seq_very_lv_threshold": 36,
            "colossus_seq_max_seq_length": 2000,
            "filter_func": 'filter_lv'
        }
            ...
            kconf_configs=[
                {
                    ...
                    "json_path": k,
                    "default_value": v,
                } for k, v in colossus_seq_config.items()
            ]
        )
        self\
        .colossus_ckpt_reader(
            service_name="grpc_colossusSimV2",
            output_attr="colossus_resp",
            output_epoch_attr="read_colossus_epoch",
            read_thread_num=32,
            buffer_size= 100000,
            batch_size = 512,
            use_common_colossus_resp=True,
            user_id_attr="user_id",
        )\
        .if_("user_id < 0 or (user_id > (1<<60)-1)")\
            .return_(2, "invalid user_id")\
        .end_if_()\
        .gsu_common_colossus_resp_retriever(
            colossus_resp_attr="colossus_resp",
            to_common_attr=False,
            print_item_fields=False,
        ...
        .sort(score_from_attr="timestamp")\
        .copy_user_meta_info(save_current_time_ms_to_attr="_cur_time_ms_")\
        .pack_item_attr(
            item_source = {
                "reco_results": True,
            },
            mappings = [
                {
                    "from_item_attr": "timestamp",
                    "to_common_attr": "ts_list",
                },
            ],
            total_limit=10,
        )\
        .gen_common_attr_by_lua(
            attr_map={
                "last_timestamp": "ts_list[1] or 0"
            }
        )
        # 如果序列最新的样本间隔当前时间大于 24h，说明没有新的样本聚合，干掉避免重复训练
        self.if_("_cur_time_ms_ / 1000 - last_timestamp > 86400") \
            .return_(2, "last_timestamp is too old")\
        .end_if_()
        self.reset_user_meta_info(
            timestamp_attr="last_timestamp", 
            user_id_attr="user_id", 
            time_unit="s"
        )\
        .copy_user_meta_info(save_request_time_to_attr="_REQ_TIME_")\
        .enrich_attr_by_lua(
            import_common_attr = ["filter_func", "colossus_seq_ev_threshold", "colossus_seq_lv_threshold", "colossus_seq_inev_threshold", "colossus_seq_very_lv_threshold"],
            import_item_attr = ["play_time", "duration"],
            function_for_item = "calculate",
            export_item_attr = ["remove_flag"],
            lua_script = f"""
            -- 单位: s
            function is_ev(play_time, duration) 
                local res = 0;
                if (duration < colossus_seq_ev_threshold) then
                    res  = (play_time >= colossus_seq_ev_threshold);
                elseif (duration > colossus_seq_lv_threshold) then
                    res = (play_time >= colossus_seq_lv_threshold);
                else
                    res = (play_time >= duration);
                end
                return res;
            end

            function is_lv(play_time, duration)
                local res = 0;
                if (duration < colossus_seq_inev_threshold) then
                    res = (play_time >= colossus_seq_lv_threshold);
                elseif (duration > colossus_seq_very_lv_threshold) then
                    res = (play_time >= colossus_seq_very_lv_threshold);
                else
                    res = ((33 * play_time) >= (duration * 28 + 180));
                end
                return res;
            end

            function calculate()
                if filter_func == 'filter_ev' then
                    return is_ev(play_time, duration);
                    end
                if filter_func == 'filter_lv' then
                    return is_lv(play_time, duration);
                end
                return 0;
            end
            """
        ...
        .filter_by_attr(
            attr_name="remove_flag",
            remove_if=">",
            compare_to=0,
        )\
        .shuffle()\
        .truncate(size_limit="{{colossus_seq_max_seq_length}}")
        return self
...
class Retr_and_Write_Subflow(
    ...
    def _retrieve_ann_and_write(self, target_sub_flow_id):
        ...
        self.delegate_retrieve( 
            ...
            recv_common_attrs=["sim_photo_ids", "sim_photo_scores"], 
        ...
        .enrich_attr_by_lua(
            import_common_attr=["sim_photo_ids", "sim_photo_scores"],
            export_common_attr=[
                "sim_photo_id_num",
                "sim_photo_score_num",
            ],
            ...
            lua_script="""
            function calc()
                local sim_photo_id_num = 0;
                local sim_photo_score_num = 0;
                if sim_photo_ids ~= nil then
                    sim_photo_id_num = #sim_photo_ids;
                end
                if sim_photo_scores ~= nil then
                    sim_photo_score_num = #sim_photo_scores;
                end
                return sim_photo_id_num, sim_photo_score_num;
            end
            """,
        ) 
        self.if_("sim_photo_id_num == sim_photo_score_num and sim_photo_id_num > 0") \
...
btq_config = {
    "slot": 17,
    "queue_prefix":'bigcode_index_emb',
    "shard_num": 1
}
...
# 从 colossus 中读取 user 历史观看视频
colossus_update_flow = I2IRunnerFlow(name="colossus_update_flow") \
    ._retrieve_by_colossus() \
    ._get_remote_embedding(emb_server_config=emb_server_config)\
    ._get_sub_flow_id()\
    ._executor_sub_retr_flow(subflow_dict)

LeafService.CHECK_UNUSED_ATTR = False
runner = OfflineRunner("i2i_bigcode_emb_runner")
...
# runner.add_leaf_flows(leaf_flows=[colossus_update_flow], name="colossus_update_flow", thread_num=8)
